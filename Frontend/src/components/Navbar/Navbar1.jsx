  ...
  return (
    ...
    <nav className={`w-full fixed z-50 left-0 top-0  bg-white transition-all duration-300 rounded-b-lg dark:bg-[#0f172a] text-gray-900 dark:text-gray-100 shadow-sm dark:shadow-none `}>
        <div className='max-w-7xl mx-auto flex  items-center justify-between px-4 py-4 flex-nowrap'>
            ...
            {/* Center division nav menu desktop version */}
            <ul className='hidden lg:flex gap-6 text-lg bg-white p-2 rounded-full border border-[#1A1A1A] animate-in dark:bg-gray-800 '>
            {navItems.map((item,index)=>(
                    <li key={index} className='cursor-pointer relative group  ' >                    
                        {!item.children && (
                        <Link to={item.url} 
        ...
        {/* MOBILE NAV */}
      {hoverOpen && (
        <div className="lg:hidden bg-white shadow-lg py-3 px-4 space-y-3 dark:bg-gray-800 ">
          {navItems.map((item) => (
            <div key={item.id}>
              {!item.children && (
                <Link to={item.url} className="block py-2 text-gray-900 dark:text-white">
